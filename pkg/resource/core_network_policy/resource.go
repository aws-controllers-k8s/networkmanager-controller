...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.CoreNetworkID = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["alias"]
	if f0ok {
		r.ko.Status.Alias = aws.String(f0)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["coreNetworkID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: coreNetworkID"))
	}
	r.ko.Spec.CoreNetworkID = &primaryKey

	f0, f0ok := fields["alias"]
	if f0ok {
		r.ko.Status.Alias = aws.String(f0)
	}
